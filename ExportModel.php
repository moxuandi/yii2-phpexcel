...
/**
 * 基于 PHPExcel, 用于将数据模型导出为 Excel 表格.
 *
...
class ExportModel extends Widget
{
    ...
    private function exportModel()
    {
        ...
        if(!empty($this->properties)){
            self::setProperties($sheet, $this->properties);  // 设置 Excel 文件的属性
        }

        if($this->isMultipleSheet){  // 导出多个工作表
            $sheetIndex = 0;
            foreach($this->models as $key => $models){
                $worksheet = $sheetIndex >= 1 ? $sheet->createSheet($sheetIndex) : $sheet->getActiveSheet();
            ...
            $worksheet = $sheet->getActiveSheet();  // 获取活动的表
    ...
    private function executeColumns(&$activeSheet, $models, $columns = [], $headers = [])
    {
        ...
        foreach($models as $model){
            ...
            foreach($columns as $column){
                ...
                $activeSheet->setCellValueExplicit($col . $row, $value, \PHPExcel_Cell_DataType::TYPE_STRING2);  // 设置单元格的值, 防止长数字变成科学计数法
                $colNum++;
            }
            $row++;
        }
