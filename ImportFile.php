...
class ImportFile extends Widget
{
    ...
    {
        $objectPhpExcel = \PHPExcel_IOFactory::load($this->file);  // 载入 Excel 表格
        $sheetCount = $objectPhpExcel->getSheetCount();  // 获取 Excel 中工作表的数量
        $sheetData = [];
        if(is_string($this->getOnlySheet)){
            $sheetData = $objectPhpExcel->getSheetByName($this->getOnlySheet)->toArray(null, true, true, true);  // 返回一个二维数组, 数组键是行的id, 子数组键是列的大写英文字母
            if($this->setFirstRecordAsKeys){
                $sheetData = self::setFirstRecordAsLabel($sheetData);  // 将第一行记录设置为每行数据的键
            }
        }elseif($sheetCount <= 1){
            $sheetData = $objectPhpExcel->getActiveSheet()->toArray(null, true, true, true);  // 返回一个二维数组, 数组键是行的id, 子数组键是列的大写英文字母
            if($this->setFirstRecordAsKeys){
                $sheetData = self::setFirstRecordAsLabel($sheetData);  // 将第一行记录设置为每行数据的键
            }
        }else{
            foreach($objectPhpExcel->getSheetNames() as $sheetIndex => $sheetName){
                if($this->setIndexSheetByName){
                    ...
                    $sheet = $objectPhpExcel->getSheetByName($indexed);  // 按表名获取表
                }else{
                    $indexed = $sheetIndex;  // 索引类型: 数字索引
                    $sheet = $objectPhpExcel->getSheet($indexed);  // 按索引获取工作表
                }
